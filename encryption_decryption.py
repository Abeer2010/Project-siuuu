...
def saveData():
    ...
    byte_str = bytes.fromhex(ciphercode)
    print(byte_str)
    write(byte_str)
    file_name_entry.delete("0.0",END)
...
heading_label = Label(root, text="Encryption & Decryption" , font = 'arial 18 italic',bg = '#AB92BF')
heading_label.place(relx=0.5,rely=0.2, anchor=CENTER)

btn_start_encryption = Button(root, text="Start Encryption" , font = 'arial 13' , command=startEncryption,bg ='#B565A7',relief=FLAT )
btn_start_encryption.place(relx=0.3,rely=0.6, anchor=CENTER)

btn_start_decryption = Button(root, text="Start Decryption" , font = 'arial 13' ,  command=startDecryption,bg ='#B565A7',relief=FLAT)
